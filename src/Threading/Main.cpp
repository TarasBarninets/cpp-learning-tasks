...
static size_t WriteToFileCallback(void* chunk_data, size_t size, size_t chunk_size, void* our_file)
{
	...
	size_t chunkSize = size * chunk_size;
...
void save_without_buffer(const char* url, ofstream* file)
{
...
static size_t WriteMemoryCallback(void* chunk_data, size_t size, size_t chunk_size, void* ourBufferToSave)
{
	static size_t callNumber = 0;
	size_t chunkSize = size * chunk_size;
	...
	callNumber++;
	mem->buffer = (char*)realloc(mem->buffer, mem->size + chunkSize + 1); //If want to download img - reduce - 1 - without 0\ ->B
...
void save_in_buffer(char* url, BufferStruct* output)
{
	CURL* httpHandle = curl_easy_init(); // Start a libcurl easy session, This function must be the first function to call. Return CURL easy handle

	curl_easy_setopt(httpHandle, CURLOPT_WRITEFUNCTION, WriteMemoryCallback); // Passing the function pointer to LC
...
void save_to_file(const char* url, const char* file_name) // without own callback
{
	CURL* httpHandle = curl_easy_init(); // Start a libcurl easy session, This function must be the first function to call. Return CURL easy handle
...
int main(int argc, char** argv)
{
	...
	thread Thread1(save_in_buffer, argv[1], &output);
	thread Thread2(save_to_file, argv[2], argv[3]);

	ofstream file;
	file.open("E:/Source/cpp-learning-tasks/src/Threading/FileFromThread3.txt", ios::out | ios::app); // ios::out | ios::app - will write from end of file. If file have some text, new text will write after
	thread Thread3(save_without_buffer, argv[2], &file);
	...
	ofstream myfile;
	myfile.open("E:/Source/cpp-learning-tasks/src/Threading/FileFromThread1.txt", ios::out);
	myfile.write(output.buffer, output.size);
	myfile.close();
